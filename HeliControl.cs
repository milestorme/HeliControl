...
{
    [Info("HeliControl", "Shady", "1.4.7", ResourceId = 1348)]
    ...
    internal class HeliControl : RustPlugin
    {
        ...
        public void DieInstantly(BaseCombatEntity heli)
        {
            HitInfo hitinfo = new HitInfo();
            heli.health = 0f;
            heli.lifestate = global::BaseCombatEntity.LifeState.Dead;
            heli.OnDied(hitinfo); //  OnKilled(null) => onDied(hitinfo)
        }
        ...
        private void cmdStrafeHeli(IPlayer player, string command, string[] args)
        {
            ...
            if (isAll)
            {
                foreach (var heli in _PatrolHelicopters)
                {
                    ...
                    if (ai != null) StartStrafe(ai, target, targPos, ai.CanUseNapalm());
                }
            }
            else StartStrafe(HeliInstance, target, targPos, HeliInstance.CanUseNapalm());
        ...
        private void StartStrafe(PatrolHelicopterAI heli, BasePlayer player, Vector3 target, bool useNapalm)
        {
            ...
            heli.State_Strafe_Enter(player, useNapalm);
        }
        ...
        {
            if (string.IsNullOrEmpty(name)) throw new ArgumentNullException(nameof(name));
            var matches = Facepunch.Pool.Get<List<BasePlayer>>();
            try
            {
            ...
            finally { Facepunch.Pool.FreeUnmanaged(ref matches); }
        }
        ...
        {
            var perms = Facepunch.Pool.Get<List<string>>();
            var time = -1f;
            try
            {
                ...
                if (perms.Count > 0)
                {
                    var nums = Facepunch.Pool.Get<List<float>>();
                    try
                    {
                    ...
                    finally { Facepunch.Pool.FreeUnmanaged(ref nums); }
                }

                return time;
            }
            finally { Facepunch.Pool.FreeUnmanaged(ref perms); }
        }
        ...
        private int GetHighestLimit(BasePlayer player, bool ch47 = false)
        {
            ...
            try
            {
                ...
                if (perms.Count > 0)
                {
                    var nums = Facepunch.Pool.Get<List<int>>();
                    try
                    {
                        for (int i = 0; i < perms.Count; i++)
                        {
